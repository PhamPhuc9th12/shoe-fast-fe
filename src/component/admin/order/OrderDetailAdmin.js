...
import { useHistory, useParams } from "react-router-dom";
import { toast } from "react-toastify";
import { getOrderById, getOrderDetailByOrderId, updateProcess, updateSuccess, updateCancel, updateShip } from "../../../api/OrderApi";
          ...
          <div className="row mb-5">
            ...
            {order.orderStatusName !== "Đã giao" && order.orderStatusName !== "Đã hủy" && (
              <div className="d-flex justify-content-center mb-3">
                ...
                {order.orderStatusName === "Đang vận chuyển" && (
                  <button className="btn btn-warning mx-2" onClick={() => setShowThird(true)}>
                    Xác nhận đã giao hàng
                  </button>
                )}
                {order.orderStatusName !== "Đang vận chuyển" && (
                  <button className="btn btn-danger mx-2" onClick={() => setShowFouth(true)}>
                    Hủy đơn hàng
                  </button>
                )}
              </div>
      ...
      <Modal show={showThird} onHide={handleCloseThird}>
        ...
        <Modal.Body>
          <Alert variant="success">
            ...
            <hr />
            <p className="font-weight-bold">
              Tổng tiền đơn hàng: {order && order.total?.toLocaleString()} đ
            </p>
          </Alert>
          <Form.Group className="mb-3" controlId="formBasicCheckbox">
            <Form.Check
