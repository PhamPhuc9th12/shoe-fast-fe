...
import fourth from "../static/images/slider_5_image.jpg";
import React, { useState, useEffect } from "react";
import { getAllProducts, toggleLikeProduct } from "../api/ProductApi"; // Thêm import toggleLikeProduct
...
const Home = (props) => {
  ...
  const handleLike = (productId, currentLikeStatus) => {
    ...
    toggleLikeProduct(productId, !currentLikeStatus, token).then((response) => {
      const updatedProducts = products.map(product =>
        product.id === productId ? { ...product, liked: !currentLikeStatus } : product
      );
